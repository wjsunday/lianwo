        ...
        for($i=0;$i<count($list['data']);$i++)
        {
            if ($list['data'][$i]['data']['type'] == 'postimage')
            {
                 for($j=0;$j<count($list['data'][$i]['data']['attach_id']);$j++)
                 {
                     ...
                     $photo_and_video[$k]['type'] = 'image';
                     $photo_and_video[$k]['image_url'] = getImageUrl($attachInfo['save_path'].$attachInfo['save_name']);
                     $photo_and_video[$k]['video_url'] = "";
                 }
                ...
                if ($video_id) {
                    $photo_and_video[$k]['type'] = 'video';
                    $photo_and_video[$k]['image_url'] = $video_server.$list['data'][$i]['data']['image_path'];
                    if ($list['data'][$i]['data']['transfer_id'] && !D('video_transfer')->where('transfer_id='.$list['data'][$i]['data']['transfer_id'])->getField('status')) {
                        $video_list[$k]['transfering'] = 1;
                    } else {
                        $photo_and_video[$k]['video_url'] = $list['data'][$i]['data']['video_mobile_path'] ? $video_server.$list['data'][$i]['data']['video_mobile_path'] : $video_server.$list['data'][$i]['data']['video_path'];
                    }
                }
